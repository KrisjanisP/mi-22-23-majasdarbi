...
action prompt_action(const vector<int>& curr_numbers) {
    ...
    for(int i=0;i<7;i++)
        cout<<"\t"<<actions[i]<<endl;
    int result;
    input("Izvēlieties darbību [1-7]: ",result,[](int x)->bool {return x>=1&&x<=7;});
...
void do_queue_action(const vector<int>& curr_numbers) {
    queue<int> rinda;
    for(int x: curr_numbers) rinda.push(x);
    vector<int> result;
    while(!rinda.empty()) {
...
void do_stack_action(const vector<int>& curr_numbers) {
    stack<int> steks;
    for(int x: curr_numbers) steks.push(x);
    vector<int> result;
    while(!steks.empty()) {
...
void do_double_stack_action(const vector<int>& curr_numbers) {
    stack<int> steki[2];
    for(int x: curr_numbers) steki[0].push(x);
    while(!steki[0].empty()) {
...
void do_list_action(const vector<int>& curr_numbers) {
    list<int> saraksts;
    for(int x: curr_numbers) saraksts.push_back(x);
    vector<int> result;
