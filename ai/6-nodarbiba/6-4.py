    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if len(self.train_mean_list):
                self.train_mean = torch.mean(
                    torch.stack(self.train_mean_list), dim=0)
                self.train_var = torch.mean(
                    torch.stack(self.train_var_list), dim=0)
                self.train_mean_list.clear()
                self.train_var_list.clear()

        out = (x - self.train_mean) / torch.sqrt(self.train_var + 1e-8)
...
class LossBCE(torch.nn.Module):
    ...
    def forward(self, y_prim, y):
        y_idx = (y.cpu().numpy() > 0.5).astype(np.int32)
        w = torch.FloatTensor(dataset_full.Y_weights[y_idx]).unsqueeze(
            dim=-1).to(y.device)
        loss = -torch.mean(w * y * torch.log(y_prim + 1e-8) +
                           (1 - y) * torch.log(1 - y_prim + 1e-8))
        return loss
...
for epoch in range(1, 1000):
    ...
    for dataloader in [dataloader_train, dataloader_test]:
        ...
        for x, x_classes, y_idx in dataloader:
            ...
            y_prim_idx = np.round(
                y_prim_idx.cpu().detach().numpy()).astype(np.int32)
