...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            MaxPool2d(kernel_size=4, stride=2, padding=1),
            Conv2d(in_channels=9,out_channels=1,kernel_size=3,stride=1,padding=1),
        )

        out_1 = get_out_size(dataset_full.input_size, kernel_size=3, stride=1, padding=1)
        out_2 = get_out_size(out_1, kernel_size=3, stride=1, padding=1)
        out_3 = get_out_size(out_2, kernel_size=4, stride=2, padding=1)
        out_4 = get_out_size(out_3, kernel_size=3, stride=1, padding=1)
        out_5 = get_out_size(out_4, kernel_size=4, stride=2, padding=1)
        out_6 = get_out_size(out_5, kernel_size=3, stride=1, padding=1)
