...
class DenseNet(torch.nn.Module):
    def __init__(self):
        ...
        self.linear = torch.nn.Linear(
            in_features=288, out_features=len(dataset_full.labels))
...
class SEBlock(torch.nn.Module):
    def __init__(self, channels, reduction=8):
        ...
        self.fc_1 = torch.nn.Linear(
            in_features=channels, out_features=round(channels / reduction))
        self.fc_2 = torch.nn.Linear(
            in_features=round(channels / reduction), out_features=channels)
...
class SEResBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, reduction=8):
        ...
        self.seblock = SEBlock(channels=out_channels, reduction=reduction)
